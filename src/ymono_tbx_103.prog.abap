...
*& Title            : CKM3N sync.(with PP)
*&---------------------------------------------------------------------*
*& Description      : CKM3N üzerinden toplu malzeme listeleme sync.(w)
*&---------------------------------------------------------------------*
*& CHANGE HISTORY:
*& Version Date.      Author       Description
*&
*&---------------------------------------------------------------------*
REPORT ymono_tbx_103.
...
SELECTION-SCREEN BEGIN OF BLOCK b1 WITH FRAME TITLE TEXT-s01.
  ...
  PARAMETERS: P_async TYPE c RADIOBUTTON GROUP r1,
              p_sync  TYPE c RADIOBUTTON GROUP r1.
SELECTION-SCREEN END OF BLOCK b1.

DATA : gv_send_jobs TYPE i.
DATA : gv_receive_jobs TYPE i.
DATA : gv_running TYPE i.
...
*&---------------------------------------------------------------------*
FORM get_data .
  CONSTANTS : lc_group TYPE rzllitab-classname
                       VALUE 'parallel_generators'.         "#EC NOTEXT
  ...
  DATA : lv_max_wps     TYPE i,
         lv_free_wps    TYPE i,
         lv_jobcount(4) TYPE n.
  DATA : lv_task TYPE char30.
...
** Parallel processing
  ...
  CLEAR lv_end.
  IF p_async = 'X'.
    CLEAR : gv_send_jobs,gv_receive_jobs,gv_running.
    DO.
...
* min 5 wps must be free
      ...
      CONCATENATE 'ZCO26' lv_jobcount INTO lv_task.
      gv_send_jobs = gv_send_jobs + 1.
      REFRESH lt_kalnr.
      gv_running = gv_running + 1.
      APPEND LINES OF zlt_kalnr FROM lv_beg TO lv_end
                   TO lt_kalnr.
      IF lt_kalnr[] IS INITIAL.
        EXIT.
      ENDIF.
      ycount = lv_beg.
      CONCATENATE ycount '/' tcount  'malzeme işlendi' INTO ytext.

      CALL FUNCTION 'SAPGUI_PROGRESS_INDICATOR'
        EXPORTING
          text = ytext.
      CALL FUNCTION 'ZCO_MLCCS_KEPH_MLCD_READ'
        STARTING NEW TASK lv_task
        DESTINATION IN GROUP lc_group
        PERFORMING fm_cb_get_data ON END OF TASK
        EXPORTING
          i_refresh_buffer      = 'X'
          ...
          ir_curtp              = lr_curtp[]
        EXCEPTIONS
          communication_failure = 1 MESSAGE message_failure
          system_failure        = 2 MESSAGE message_failure
          resource_failure      = 3
          OTHERS                = 99.
      IF sy-subrc <> 0.
      ...
      lv_beg = lv_end + 1.
    ENDDO.
    WAIT UNTIL gv_receive_jobs >= gv_send_jobs UP TO 18000 SECONDS.
  ELSE.
...
*&      Form  FM_CB_GET_DATA
*&---------------------------------------------------------------------*
FORM fm_cb_get_data  USING gv_taskname TYPE any. "#EC CALLED "#EC NEEDED
  DATA : lt_keph_mlcd TYPE ccs01_t_keph_mlcd WITH HEADER LINE.
  "zco_ccs01_keph_mlcd_tt.
  DATA : ls_keph_mlcd TYPE LINE OF ccs01_t_keph_mlcd.
  gv_receive_jobs = gv_receive_jobs + 1.
  RECEIVE RESULTS FROM FUNCTION 'ZCO_MLCCS_KEPH_MLCD_READ'
        TABLES
  ...
  LOOP AT lt_keph_mlcd INTO ls_keph_mlcd.
    COLLECT ls_keph_mlcd INTO gt_keph_mlcd.
    IF ls_keph_mlcd-bvalt IS NOT INITIAL.
      CLEAR lt_alproc_texts.
      lt_alproc_texts-kalnr = ls_keph_mlcd-bvalt.
      COLLECT lt_alproc_texts.
    ENDIF.
  ENDLOOP.
ENDFORM.                    " FM_CALL_BACK_SARF
